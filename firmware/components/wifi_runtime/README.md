...
切换配置先停止当前 station，再应用新参数并启动。SDK 回调只投递事件，不执行存储、重连或阻塞任务。取得 IP 后重新读取当前 AP 与 netif，核对 SSID、认证类型和有效地址；旧 IP 事件不能直接证明候选成功。停机超过 2 秒或事件队列溢出时报告 failed。
...
本轮仅验证当前 C3 与实际使用的 WPA2 网络；WPA3、物理断电、其他芯片及长稳尚未验收。模块不创建 MQTT/FRP 客户端，不配置 GPIO，不输出密码。
