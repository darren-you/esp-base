...
[嵌入式标准](https://github.com/darren-you/darren-space/blob/master/harness/docs/workspace/standards/embedded_firmware/embedded_firmware_golden_path.md)。测试在 `tests/`，公开主机调用示例在固件根之外的 [tools/](../tools/README.md)。Component Manager 依赖由 `dependencies.lock` 固定；`mqtt` 唯一来源是公开 `esp-mqtt@ccf81df2215cfddd87aff97afdd2e7f17e50fbaa`，`esp_ota` 唯一来源是公开 `esp-ota@3c3f72b823ce856b02f838fef17db1368e6d5448`，`esp_frp` 唯一来源是公开 `esp-frp@c56a0f32d96c75fd28e2c04146383348d8ce2829`。host tests 使用同一已解析 cJSON、`eota.h` 与 `esp_frp.h`，不读取相邻仓。
