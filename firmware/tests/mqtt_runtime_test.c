...
{
    return (ebase_mqtt_config_t){.hostname = "broker.example.invalid", .port = 8883, .tls = true,
        .client_id = "11111111-1111-4111-8111-111111111111", .username = "test", .password = "test-only",
...
static ebase_mqtt_event_t output;
static void connect_ready(ebase_mqtt_runtime_t *r)
{
    emit(MQTT_EVENT_CONNECTED, (esp_mqtt_event_t){0});
    assert(esp_base_mqtt_poll(r, &output) && output.kind == EBASE_MQTT_EVENT_CONNECTED);
    assert(esp_base_mqtt_state(r) == EBASE_MQTT_SUBSCRIBING);
    char grant = 1;
    ...
    assert(esp_base_mqtt_state(r) == EBASE_MQTT_READY);
}

int main(void)
{
    ebase_mqtt_config_t c = config();
    ebase_mqtt_runtime_t *r = NULL;
    for (unsigned fail = 1; fail <= 2; ++fail) {
        queue_attempt = 0; fail_queue_attempt = fail;
        assert(esp_base_mqtt_create(&c, &r) == ESP_ERR_NO_MEM && !r && !live_queues && !live_client);
    }
    fail_queue_attempt = 0; init_fail = true;
    assert(esp_base_mqtt_create(&c, &r) == ESP_ERR_NO_MEM && !live_queues); init_fail = false;
    register_error = ESP_FAIL;
    assert(esp_base_mqtt_create(&c, &r) == ESP_FAIL && !live_queues && !live_client); register_error = 0;
    assert(esp_base_mqtt_create(&c, &r) == ESP_OK);
    assert(esp_base_mqtt_create(&c, &r) == ESP_ERR_INVALID_STATE);
    ...
    assert(esp_base_mqtt_start(r, false, true) == ESP_ERR_INVALID_STATE);
    assert(esp_base_mqtt_start(r, true, false) == ESP_ERR_EBASE_MQTT_TIME_REQUIRED);
    start_error = ESP_FAIL; assert(esp_base_mqtt_start(r, true, true) == ESP_FAIL); start_error = 0;
    assert(esp_base_mqtt_start(r, true, true) == ESP_OK);
    current_task = 2; assert(esp_base_mqtt_stop(r) == ESP_ERR_INVALID_STATE); current_task = 1;
    connect_ready(r);
    int id = -100;
    enqueue_result = -2;
    assert(esp_base_mqtt_enqueue(r, "unit/out", "x", 1, 1, false, &id) == ESP_ERR_EBASE_MQTT_OUTBOX_FULL && id == -100);
    enqueue_result = -1; assert(esp_base_mqtt_enqueue(r, "unit/out", "x", 1, 1, false, &id) == ESP_FAIL);
    enqueue_result = 0;
    assert(esp_base_mqtt_enqueue(r, "unit/out", "not-a-payload", 0, 0, false, &id) == ESP_OK && id == 0);
    assert(sent_data == NULL && sent_length == 0 && sent_qos == 0);
    assert(esp_base_mqtt_enqueue(r, "unit/+", "x", 1, 1, false, &id) == ESP_ERR_INVALID_ARG);
    assert(esp_base_mqtt_enqueue(r, "unit/out", "x", 4097, 1, false, &id) == ESP_ERR_INVALID_ARG);
    assert(esp_base_mqtt_outbox_size(r) == 123);
    char bytes[] = "abcdef";
    emit(MQTT_EVENT_DATA, (esp_mqtt_event_t){.msg_id = 10, .topic = "unit/in", .topic_len = 7,
        .data = bytes, .data_len = 3, .total_data_len = 6, .qos = 1});
    assert(!esp_base_mqtt_poll(r, &output));
    emit(MQTT_EVENT_DATA, (esp_mqtt_event_t){.msg_id = 10, .data = bytes + 3,
        .data_len = 3, .total_data_len = 6, .current_data_offset = 3, .qos = 1});
    memset(bytes, 'X', sizeof(bytes));
    assert(esp_base_mqtt_poll(r, &output) && output.kind == EBASE_MQTT_EVENT_MESSAGE);
    assert(output.message.length == 6 && !memcmp(output.message.payload, "abcdef", 6));
    emit(MQTT_EVENT_DELETED, (esp_mqtt_event_t){.msg_id = 41});
    assert(esp_base_mqtt_poll(r, &output) && output.kind == EBASE_MQTT_EVENT_DELETED && output.error == EBASE_MQTT_ERROR_EXPIRED);
    assert(esp_base_mqtt_state(r) == EBASE_MQTT_READY);
    ...
    assert(esp_base_mqtt_poll(r, &output) && output.kind == EBASE_MQTT_EVENT_READY);
    assert(esp_base_mqtt_unsubscribe(r, "unit/extra") == ESP_OK);
    emit(MQTT_EVENT_UNSUBSCRIBED, (esp_mqtt_event_t){.msg_id = 32});
    assert(esp_base_mqtt_poll(r, &output) && output.kind == EBASE_MQTT_EVENT_UNSUBSCRIBED);
    ...
    assert(esp_base_mqtt_poll(r, &output) && output.error == EBASE_MQTT_ERROR_SUBSCRIPTION);
    assert(!sdk.started && esp_base_mqtt_state(r) == EBASE_MQTT_FAILED);
    assert(!esp_base_mqtt_poll(r, &output));
    assert(esp_base_mqtt_start(r, true, true) == ESP_OK);
    emit(MQTT_EVENT_CONNECTED, (esp_mqtt_event_t){0}); assert(esp_base_mqtt_poll(r, &output));
    now_us += 10000001;
    ...
    assert(esp_base_mqtt_poll(r, &output) && output.error == EBASE_MQTT_ERROR_QUEUE && !sdk.started);
    assert(!esp_base_mqtt_poll(r, &output));
    assert(esp_base_mqtt_start(r, true, true) == ESP_OK); connect_ready(r);
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        assert(esp_base_mqtt_stop(r) == ESP_OK);
        assert(esp_base_mqtt_start(r, true, true) == ESP_OK); connect_ready(r);
        assert(esp_base_mqtt_destroy(r) == ESP_OK && !live_client && !live_queues);
    }
