...
static void digest(esp_base_ota_update_request_t *request)
{
    uint32_t sum = 0;
...
{
    assert(partition == &new_slot && offset + size <= staged_size);
    ++partition_reads;
...
esp_http_client_handle_t esp_http_client_init(const esp_http_client_config_t *config)
{
    ...
    assert(config->disable_auto_redirect && config->timeout_ms == 5000);
    return (void *)1;
}
esp_err_t esp_http_client_open(esp_http_client_handle_t client, int write_len)
{ assert(client && write_len == 0); ++open_calls; return ESP_OK; }
esp_err_t esp_http_client_set_timeout_ms(esp_http_client_handle_t client, int timeout_ms)
{ assert(client && timeout_ms == 1000 && open_calls == 1); return ESP_OK; }
...
int main(void)
{
    esp_base_ota_update_request_t request = {.image_url = "https://example.test/esp-base.bin", .image_size_bytes = IMAGE_BYTES};
    assert(esp_base_ota_update_available());
    reset(); digest(&request);
    assert(esp_base_ota_update_run(&request, progress, NULL) == ESP_BASE_OTA_UPDATE_OK);
    assert(begin_calls == 1 && write_calls > 1 && staged_size == IMAGE_BYTES);
    assert(memcmp(staged_bytes, image_bytes, IMAGE_BYTES) == 0);
    assert(partition_reads == 2 && end_calls == 1 && abort_calls == 0 && cleanup_calls == 1);
    assert(select_calls == 1 && last_progress == IMAGE_BYTES && boot == &new_slot);
    reset(); valid_old = false;
    assert(esp_base_ota_update_run(&request, progress, NULL) == ESP_BASE_OTA_UPDATE_SLOT_UNAVAILABLE && init_calls == 0);
    ...
    assert(esp_base_ota_update_run(&request, progress, NULL) == ESP_BASE_OTA_UPDATE_OK && boot == &new_slot);
    reset(); selected_slot = &wrong_slot;
    assert(esp_base_ota_update_run(&request, progress, NULL) == ESP_BASE_OTA_UPDATE_SLOT_UNAVAILABLE && init_calls == 0);
    reset(); request.image_size_bytes = new_slot.size + 1;
    assert(esp_base_ota_update_run(&request, progress, NULL) == ESP_BASE_OTA_UPDATE_TOO_LARGE && init_calls == 0);
    request.image_size_bytes = IMAGE_BYTES;
    reset(); image_bytes[sizeof(esp_image_header_t) + sizeof(esp_image_segment_header_t) + 48] = 'x';
    assert(esp_base_ota_update_run(&request, progress, NULL) == ESP_BASE_OTA_UPDATE_WRONG_TARGET && begin_calls == 0 && select_calls == 0);
    reset(); bad_chip = true;
    assert(esp_base_ota_update_run(&request, progress, NULL) == ESP_BASE_OTA_UPDATE_WRONG_TARGET && begin_calls == 0);
    reset(); image_bytes[0] = 0;
    assert(esp_base_ota_update_run(&request, progress, NULL) == ESP_BASE_OTA_UPDATE_WRONG_TARGET && begin_calls == 0);
    reset(); content_length = -1;
    assert(esp_base_ota_update_run(&request, progress, NULL) == ESP_BASE_OTA_UPDATE_DOWNLOAD_FAILED && read_calls == 0 && begin_calls == 0);
    reset(); status_code = 302;
    assert(esp_base_ota_update_run(&request, progress, NULL) == ESP_BASE_OTA_UPDATE_DOWNLOAD_FAILED && begin_calls == 0);
    reset(); stall_headers = true;
    assert(esp_base_ota_update_run(&request, progress, NULL) == ESP_BASE_OTA_UPDATE_DOWNLOAD_FAILED && header_calls == 30 && read_calls == 0 && cleanup_calls == 1);
    reset(); stall_first_byte = true;
    assert(esp_base_ota_update_run(&request, progress, NULL) == ESP_BASE_OTA_UPDATE_DOWNLOAD_FAILED && read_calls == 30 && begin_calls == 0 && cleanup_calls == 1);
    reset(); stall_midbody = true;
    assert(esp_base_ota_update_run(&request, progress, NULL) == ESP_BASE_OTA_UPDATE_DOWNLOAD_FAILED && begin_calls == 1 && abort_calls == 1 && end_calls == 0);
    reset(); early_fin = true;
    assert(esp_base_ota_update_run(&request, progress, NULL) == ESP_BASE_OTA_UPDATE_DOWNLOAD_FAILED && begin_calls == 0 && cleanup_calls == 1);
    reset(); fin_midbody = true;
    assert(esp_base_ota_update_run(&request, progress, NULL) == ESP_BASE_OTA_UPDATE_DOWNLOAD_FAILED && begin_calls == 1 && abort_calls == 1 && select_calls == 0);
    reset(); complete = false;
    assert(esp_base_ota_update_run(&request, progress, NULL) == ESP_BASE_OTA_UPDATE_DOWNLOAD_FAILED && abort_calls == 1 && end_calls == 0);
    reset(); read_advance_us = INT64_C(25000000);
    assert(esp_base_ota_update_run(&request, progress, NULL) == ESP_BASE_OTA_UPDATE_DOWNLOAD_FAILED && now_us >= INT64_C(300000000) && end_calls == 0);
    reset(); request.sha256[0] ^= 1;
    assert(esp_base_ota_update_run(&request, progress, NULL) == ESP_BASE_OTA_UPDATE_HASH_MISMATCH && abort_calls == 1 && end_calls == 0);
    request.sha256[0] ^= 1;
    reset(); fail_read = true;
    assert(esp_base_ota_update_run(&request, progress, NULL) == ESP_BASE_OTA_UPDATE_DOWNLOAD_FAILED && abort_calls == 1 && end_calls == 0);
    reset(); end_result = ESP_ERR_OTA_VALIDATE_FAILED;
    assert(esp_base_ota_update_run(&request, progress, NULL) == ESP_BASE_OTA_UPDATE_SIGNATURE_INVALID && end_calls == 1 && select_calls == 0 && boot == &old_slot);
    reset(); fail_select = true;
    assert(esp_base_ota_update_run(&request, progress, NULL) == ESP_BASE_OTA_UPDATE_SLOT_UNAVAILABLE && select_calls == 1 && boot == &old_slot);
    reset(); select_then_fail = true;
    assert(esp_base_ota_update_run(&request, progress, NULL) == ESP_BASE_OTA_UPDATE_SLOT_UNAVAILABLE && restore_calls == 1 && boot == &old_slot);
    reset(); rollback_possible = false;
    assert(esp_base_ota_update_run(&request, progress, NULL) == ESP_BASE_OTA_UPDATE_SLOT_UNAVAILABLE && restore_calls == 1 && boot == &old_slot);
    reset(); rollback_possible = false; fail_restore = true;
    assert(esp_base_ota_update_run(&request, progress, NULL) == ESP_BASE_OTA_UPDATE_BOOT_STATE_UNKNOWN && restore_calls == 1 && boot == &new_slot);
