...
| 已装配的入口 | NVS 持久身份、复位事实、已提交配置读取/条件提交、Wi-Fi station、USB status/restart/config.set、5 秒心跳；签名构建还装配 ota.start/HTTPS OTA，pending 新槽有本地确认与失败回滚接线 | [主程序](../../firmware/apps/esp_base/main/esp_base_main.c)、[控制任务](../../firmware/components/device_protocol/esp_base_protocol.c)、[OTA](../../firmware/components/ota_runtime/esp_base_ota_update.c) |
