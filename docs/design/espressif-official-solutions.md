...
| [ESP-MQTT 仓库](https://github.com/espressif/esp-mqtt) / [公开维护仓](https://github.com/darren-you/esp-mqtt) | **隔离测试应用和普通基座均已采用公开组件**，固定 `esp-mqtt@ccf81df2215cfddd87aff97afdd2e7f17e50fbaa`；普通基座在具备已提交凭据和可信时间后启动 MQTT owner。 | 旧适配层曾验证隔离 TLS Broker；新仓已有真实核心的 Linux 隔离 Broker 断线及 QoS1 ACK/重投回归和固定 SDK C3 编译。普通基座已接入 v3 持久配置、HMAC 命令与结果通道；真实设备 Broker/TLS、命令 ACK、账户 ACL 和配置迁移仍待验收。`device_id`、`boot_id`、期限、队列与最终结果由[自有协议](./device-protocol.md)裁决。 |
