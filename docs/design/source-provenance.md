...
MQTT 从公开 [esp-space/esp-mqtt](https://github.com/esp-space/esp-mqtt) 的完整提交 `9d6d95e779f4f5ff387a6d9b54015bf4e43565f2` 获取，组件名仍为 `mqtt`，在两目标依赖锁中固定。该仓以官方 [ESP-MQTT v1.1.0](https://github.com/espressif/esp-mqtt/tree/1a1e5788a5cf57a0f44a3c6c061407f6c9be1026) 为基线并保留 Apache-2.0 许可；本轮在其 MQTT core 断线入口修正 clean session 的旧 SUB/UNSUB outbox 重放，并以真实核心 Linux Broker 对照验证；另以真实核心 Linux Broker 验证 QoS1 丢失/重复 PUBACK、入站重投和断线后原 ID 重发。通用 `emqtt_` 运行层归该仓；Base 只保留持久 UUID、设备 Topic/LWT 与设备命令归属，不持有 `mqtt_runtime` 或官方 Registry 的第二份 MQTT 依赖。C3 实板 Broker/TLS 仍未验收。

OTA 通用机制由公开 [esp-space/esp-ota](https://github.com/esp-space/esp-ota) 完整提交 `207273188b984161362824c3344614e812016836` 提供，Component Manager 在两目标依赖锁中锁定其 `components/esp_ota`。该仓[来源记录](https://github.com/esp-space/esp-ota/blob/master/docs/design/source-provenance.md)绑定迁自 Base 的精确源码与许可；Base 仅维护产品约束、operation 收据、自检及调度。

可选固件集合适配从公开 [esp-space/esp-container](https://github.com/esp-space/esp-container) `8eb805f3f12cb3cd836e9833acb4aca878ae80e7` 的 `components/esp_container` 获取；其组件清单固定 WAMR `26c235e53e29acd8b43abe7f3b524577bd4d1ae5`。Base 在独立清单中声明这一来源，两目标常规依赖锁均不包含 Container/WAMR；目前主应用无包操作调用方。
